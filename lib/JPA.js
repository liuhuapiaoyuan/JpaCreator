 ...
 * 
var MemberSchema = {
	packageName:"cn.yunnet.jianyubujian",
	name:'aaatest',
	info:'测试数据库',
	fields:[
		{
...
var actions = ["entity","repositories","service","service.impl","controller.user","controller.admin","controller.admin.DTO","controller.user.DTO"]
//生成repositories
 ...
 */
function createFile(type,schema,cleanDir){
	...
	schema._updateIgnoreProperties = schema._updateIgnoreProperties.join(',')
	...
	if(type=='entity'){
		...
		createFileName = schema._name + "Controller"
	}else if(type =='controller.admin.DTO'){
		createFileName = schema._name + "DTO"
	}else if(type =='controller.user.DTO'){
		createFileName = schema._name + "DTO"
	}
	...
	mkdirsSync(savePath)
	var tmpFile = createPath([__dirname,'template',capitalize(type)+'.java'])
...
module.exports = {
	 ...
	 * @param {Object} schema
	 */
	createAll:function(schema,cleanDir){
		for(var i in actions){
			createFile(actions[i],schema,false)
		}
	},
	/**
	 * 清空
